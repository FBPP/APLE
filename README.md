...
### 线程不同步的根本原因
+ 线程的寄存器不共享，但是内存是共享的（除线程栈）
+ 变量存在于内存中，多个线程对同一个变量进行运算需要cpu读取同一块内存到cpu的寄存器，经过运算/修改后，再写入到内存中
	+ 例如 线程A和线程B都对一个变量n = 0做++操作，理论上n应该等于2
		+ 线程A读入 n = 0到寄存器, 线程B读入 n = 0到寄存器
		+ 线程A对寄存器中的n++， 线程B对寄存器中的n++、
		+ A将寄存器中的n写入内存，n = 1
		+ B将寄存器中的n写入内存，最终n = 1
### 1、锁
+ mutex 互斥量（互斥锁）
